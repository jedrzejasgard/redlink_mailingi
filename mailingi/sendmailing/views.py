...
def wyslij_mailing(request):
    if request.method == "POST":
        ...
        try:
            ...
            if czy_vip == 'on':
                lista_grup_redlink_handlowcy = []
                for itemRedlink in lista_grup_redlink:
                    if itemRedlink['GroupName'].startswith('VIP'):
                        lista_grup_redlink_handlowcy.append(itemRedlink)
            else:
                lista_grup_redlink_handlowcy = lista_grup_redlink
